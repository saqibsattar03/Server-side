...
getUserData = async (userId) => await knex('profile_clicks').where('user_id', userId);

getFilteredProfileViews = async (userId, startDate, endDate) => {
    return await knex('profile_clicks').where('user_id', userId).where('created_at', '>=', startDate)
...
getUserGroupByData = async (userId, dateFilter = null) => {
    ...
    var records = await getFilteredProfileViews(userId, startDate, moment().add(1, 'day').format());
...
insertData = async (data) => {
    return await knex('profile_clicks').insert(getDbFields(data));
}
